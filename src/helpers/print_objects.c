...
void	print_scene(const t_data *data)
{
	...
	printf("a_rgb:\t\t [%d,%d,%d]\n", data->scene->ambi.rgb.r,
		data->scene->ambi.rgb.g,
		data->scene->ambi.rgb.b);
	printf("c_rient:\t [%.1f,%.1f,%.1f]\n", data->scene->cam.orient.x,
		data->scene->cam.orient.y, data->scene->cam.orient.z);
	printf("c_xyz:\t\t [%.1f,%.1f,%.1f]\n", data->scene->cam.cords.x,
		data->scene->cam.cords.y, data->scene->cam.cords.z);
	printf("c_fov:\t\t %d\n", data->scene->cam.fov);
	printf("l_xyz:\t\t [%.1f,%.1f,%.1f]\n", data->scene->lght.cords.x,
		data->scene->lght.cords.y, data->scene->lght.cords.z);
	printf("l_bright:\t %.1f\n", data->scene->lght.bright);
	printf("l_rgb:\t\t [%d,%d,%d]\n", data->scene->lght.rgb.r,
		data->scene->lght.rgb.g, data->scene->lght.rgb.b);
}
